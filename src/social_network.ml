...
let get_neighbors
  ~(network : Network.t)
  ~(person : Person.t)
  ~(connections : Person.t list)
  =
  Set.filter network ~f:(fun (p1, p2) ->
    Person.equal p1 person
    && not (List.exists connections ~f:(fun p -> Person.equal p p2)))
;;

let rec traverse_components
  ~(network : Network.t)
  ~(person : Person.t)
  ~(connections : Person.t list)
  : Person.t list
  =
  let neighbors = get_neighbors ~network ~person ~connections in
  Set.fold neighbors ~init:connections ~f:(fun accu (_, neighbor2) ->
    let new_connections =
      if not (List.exists accu ~f:(fun p -> Person.equal p neighbor2))
      then accu @ [ neighbor2 ]
      else accu
    in
    traverse_components
...
let find_friend_group network ~person : Person.t list =
  let connections = [ person ] in
  traverse_components ~network ~person ~connections
;;
