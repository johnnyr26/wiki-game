  ...
  let of_file input_file =
    let connections =
      ...
      |> List.concat_map ~f:(fun s ->
        ...
        |> fun c -> match c with 
        | Some cities -> (
          List.mapi cities ~f: (fun i city -> 
        if i <> (List.length cities) - 1 then (City.of_string city, List.nth_exn cities (i + 1)) else (City.of_string city, City.of_string city))
        |> List.filter ~f: (fun (city1, city2) -> not(String.equal city1 city2))
        )
        | None -> ["", ""]
      ) in
      print_s [%message ((Connection.Set.of_list connections): Connection.Set.t)];
...
(* In order to visualize the social network, we use the ocamlgraph library to create a
   ...
   module G = Graph.Imperative.Digraph.ConcreteBidirectionalLabeled (City) (Interstate)
